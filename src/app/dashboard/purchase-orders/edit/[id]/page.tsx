...
import { Vendor, Customer, Quotation } from "@/app/types";
...
export default function EditPurchaseOrderPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const { data: poData } = await supabase
        ...
        .select(`*, purchase_order_items(*, product:products(name))`)
        .eq("id", poId)
        .eq("user_id", user.id)
        .single();
      ...
      setSelectedVendorId(poData.vendor_id);
      setSelectedCustomerId(poData.customer_id || "");
      setSelectedQuotationId(poData.quotation_id || "");
      setDeliveryAddress(poData.delivery_address || "");
      setIssueDate(poData.issue_date);
      setExpectedDate(poData.expected_date || "");
      setStatus(poData.status);
      setCurrencyCode(poData.currency_code || "HKD");
      setNotes(poData.notes || "");
      setTermsConditions(poData.terms_conditions || "");
      setDiscount(poData.discount_amount || 0);

      if (poData.purchase_order_items?.length > 0) {
        setPoItems(
      ...
      if (quotationsData) setQuotations(quotationsData as any);
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setError(
        "Failed to update purchase order. Please check all fields and try again."
      );
    } finally {
      setIsSubmitting(false);
    }
          ...
          <div className="flex items-center mb-6">
            ...
            <div>
              ...
              <p className="text-muted-foreground">
                Update purchase order {poNumber}
              </p>
            </div>
          </div>
            ...
            <form ref={formRef} onSubmit={handleSubmit} className="space-y-8">
                          ...
                          <tr key={item.id}>
                                    ...
                                    {filteredProducts.map((product) => (
                                      ...
                                      >
                                        ...
                                        <div className="text-sm text-gray-500 flex justify-between">
                                          ...
                                          <span>
                                            Cost: {formatCurrencyDisplay(product.cost_price || product.price)}
                                          </span>
                                        </div>
                                      </li>
                                    ))}
                            ...
                            <td className="px-6 py-4 whitespace-nowrap">
                              <div className="text-sm font-medium">
                                {formatCurrencyDisplay(item.quantity * item.unit_price)}
                              </div>
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap">
                              <Button
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-4 bg-gray-50 p-4 rounded-md">
                  <div className="flex justify-between">
                    ...
                    <span className="text-sm font-medium">{formatCurrencyDisplay(subtotal)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-sm text-gray-600">Tax:</span>
                    <span className="text-sm font-medium">{formatCurrencyDisplay(taxTotal)}</span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  <div className="flex justify-between border-t pt-2">
                    ...
                    <span className="text-base font-bold">{formatCurrencyDisplay(total)}</span>
                  </div>
              ...
              <div className="flex justify-end gap-3">
                ...
                <Button type="submit" disabled={isSubmitting}>
                  {isSubmitting ? "Saving..." : "Update Purchase Order"}
                </Button>
              </div>
            </form>
          </div>
